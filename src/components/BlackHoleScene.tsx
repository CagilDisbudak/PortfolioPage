...
// background starfield smears and swirls near the hole. The disk itself is a
// differentially-rotating plane (inner orbits shear ahead of outer ones) with
// filamentary fbm turbulence, Doppler beaming toward the approaching side,
// and a white-hot → amber → ember → violet temperature gradient. Scroll moves
...
const fragmentSrc = /* glsl */ `
  ...
  // Sky sampled by a (bent) escaped ray. The angular seam of the spherical
  // mapping is anchored behind the camera's horizontal forward axis so it can
  // never appear inside the field of view.
  vec3 background(vec3 rd, vec3 fwdH, float t) {
    ...
    // Seamless nebula from the 3D direction itself.
    float neb1 = fbm(vec2(rd.x * 2.0 + rd.z * 1.3, rd.y * 2.4 - rd.z * 0.7) * 2.0);
    float neb2 = fbm(vec2(rd.y * 1.7 - rd.x * 0.9, rd.z * 2.1 + rd.x * 1.4) * 2.6 + 7.0);
    col += vec3(0.15, 0.06, 0.23) * pow(neb1, 3.0) * 0.9;
    col += vec3(0.05, 0.09, 0.20) * pow(neb2, 4.0) * 0.7;
  ...
  void main() {
    ...
    if (captured < 0.5) {
      ...
      col += trans * background(dir, fwdH, t);
